...
def group_form_view(request: WSGIRequest, pk: int = 0) -> TemplateResponse:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            group.add_activity(
                user=request.user,
                verb='added',
                information=f'{request.user.get_full_name()} added {len(user_list)} users to the permission "{group.name}".'
            )
...
def group_delete_user_form_view(
    request: WSGIRequest,
    group_pk: int,
    pk: int
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            group.add_activity(
                user=request.user,
                verb='removed',
                information=f'{request.user.get_full_name()} removed {user.get_full_name()} from the permission "{group.name}".'
            )
