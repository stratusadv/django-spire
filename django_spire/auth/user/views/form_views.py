...
def user_register_form_view(request):
    ...
    crumbs = Breadcrumbs()
    crumbs.add_breadcrumb(name='Users', href=reverse('user:list'))
    ...
    return portal_views.template_view(
        ...
        breadcrumbs=crumbs,
        template='django_spire/user/page/register_user_form_page.html'
    )
...
def user_form_view(request, pk):
    ...
    return portal_views.model_form_view(
        ...
        obj=portal_user,
        context_data=context_data,
        template='django_spire/user/page/user_form_page.html'
    )


@permission_required('permission.delete_portaluser')
def user_toggle_status_form_view(request, pk):
