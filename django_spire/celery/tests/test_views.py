...
class TaskViewTestCase(TestCase):
    ...
    def test_task_view_retrieves_task(self, mock_get_object_or_404) -> None:
        ...
        response = task_view(request, self.celery_task.task_id)
    ...
    def test_task_view_updates_task_from_async_result(
        ...
        task_view(request, self.celery_task.task_id)
...
class TaskListViewTestCase(TestCase):
    ...
    def test_task_list_view_requires_login(self) -> None:
        self.client.logout()
        response = self.client.get(
            reverse(
    ...
    def test_task_list_view_retrieves_tasks(self) -> None:
        request = self.factory.get(f'/task/list/{self.reference_key}/')
        request.user = self.super_user

        response = task_list_view(request, self.reference_key)

        assert response.status_code == 200

    def test_task_list_view_uses_correct_template(self) -> None:
        request = self.factory.get(f'/task/list/{self.reference_key}/')
        request.user = self.super_user
    ...
    def test_task_list_view_passes_tasks_to_context(self) -> None:
        request = self.factory.get(f'/task/list/{self.reference_key}/')
        request.user = self.super_user
    ...
    def test_task_list_view_filters_by_reference_key(self) -> None:
    ...
    def test_task_list_view_filters_unready_only(self) -> None:
        request = self.factory.get(f'/task/list/{self.reference_key}/')
        request.user = self.super_user
    ...
    def test_task_list_view_empty_when_no_matching_tasks(self) -> None:
        request = self.factory.get('/task/list/nonexistent_key/')
        request.user = self.super_user

        response = task_list_view(request, 'nonexistent_key')
