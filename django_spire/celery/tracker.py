...
class CeleryTaskTracker:
    ...
    def __init__(self, celery_task: Task, update_interval_seconds: int = 10) -> None:
    ...
    def _process_overdue_update(self) -> None:
        if self._is_overdue_for_update():
            self._celery_task.update_state(
                state=self._state, meta={**asdict(self._meta), **(self._additional_meta or {})}
            )
    ...
    def _update_remaining_seconds(self) -> None:
        remaining_seconds = (1.0 / self._progress) * (time.time() - self._start_time_seconds)
