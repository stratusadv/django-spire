...
from django_spire.sync.core.exceptions import RecordFieldError, RecordSerializationError
...
def test_missing_field_raises() -> None:
    ...
    with pytest.raises(RecordFieldError, match='missing'):
...
def test_non_serializable_raises(hasher: RecordHasher) -> None:
    ...
    with pytest.raises(RecordSerializationError, match='non-serializable'):
        hasher.hash(record)
