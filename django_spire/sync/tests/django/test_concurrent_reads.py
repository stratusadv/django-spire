...
def test_reader_never_sees_torn_state_during_upsert() -> None:
    ...
    setup = make_storage()
    setup.upsert_many(
        'sync_tests.SyncTestModel', {key: make_named_record(key, 'initial', 100, value=0)}, ''
    )
    ...
    writers = [threading.Thread(target=thread_safe(writer, errors)) for _ in range(2)]
    readers = [threading.Thread(target=thread_safe(reader, errors)) for _ in range(3)]
...
def test_get_changed_since_returns_consistent_snapshot() -> None:
    ...
    def writer() -> None:
        ...
        while not stop.is_set():
            ...
            storage.upsert_many(
                'sync_tests.SyncTestModel', {k: make_named_record(k, f'upd-{ts}', ts, value=ts)}, ''
            )
            ts += 1
    ...
    threads = [threading.Thread(target=thread_safe(writer, errors)) for _ in range(2)] + [
...
def test_concurrent_many_to_many_assignment_consistent() -> None:
    ...
    def assign(tag_pks: list[str], ts: int) -> None:
        ...
        storage.upsert_many(
            'sync_tests.SyncTestModel',
            {
                key: SyncRecord(
                    key=key,
                    data={'id': key, 'name': 'host', 'value': 0, 'tags': tag_pks},
    ...
    threads = [
        threading.Thread(
            target=thread_safe(assign, errors, barrier=barrier), args=([str(tag_a.pk)], 200)
        ),
        threading.Thread(
            target=thread_safe(assign, errors, barrier=barrier), args=([str(tag_b.pk)], 300)
        ),
        threading.Thread(
    ...
    assert final_tags in ({tag_a.pk}, {tag_b.pk}, {tag_a.pk, tag_c.pk}), (
...
def test_reader_during_concurrent_writers_no_lost_updates() -> None:
    ...
    threads = [
        threading.Thread(target=thread_safe(writer, errors), args=(w,)) for w in range(3)
    ] + [threading.Thread(target=thread_safe(reader, errors)) for _ in range(3)]
