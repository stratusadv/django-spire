...
def test_from_dict_valid() -> None:
    record = SyncRecord.from_dict('1', {'data': {'name': 'Alice'}, 'timestamps': {'name': 100}})
...
def test_sync_field_last_modified_uses_max_timestamp() -> None:
    record = SyncRecord(
        key='1', data={'name': 'Alice', 'value': 10}, timestamps={'name': 100, 'value': 300}
    )
...
def test_sync_field_last_modified_uses_received_at_when_higher() -> None:
    record = SyncRecord(key='1', data={}, timestamps={'name': 100}, received_at=500)

    assert record.sync_field_last_modified == 500


def test_received_at_not_serialized() -> None:
    record = SyncRecord(key='1', data={'name': 'Alice'}, timestamps={'name': 100}, received_at=500)
