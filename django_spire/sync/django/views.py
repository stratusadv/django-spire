...
def _reject_if_oversized_header(
    request: HttpRequest, request_bytes_max: int
) -> JsonResponse | None:
    ...
    try:
        ...
        return JsonResponse({'ok': False, 'error': 'Invalid Content-Length header'}, status=400)
...
def _read_body(request: HttpRequest, request_bytes_max: int) -> tuple[bytes, JsonResponse | None]:
    body = request.body

    if len(body) > request_bytes_max:
        error = JsonResponse({'ok': False, 'error': 'Request body too large'}, status=413)
    ...
    try:
        ...
        error = JsonResponse(
            {'ok': False, 'error': 'Decompressed request body too large'}, status=413
        )
        ...
        error = JsonResponse(
            {'ok': False, 'error': 'Failed to decompress request body'}, status=400
        )

        return b'', error


def _parse_manifest(body: bytes) -> tuple[SyncManifest | None, JsonResponse | None]:
    try:
        data = json.loads(body)
    except (json.JSONDecodeError, UnicodeDecodeError):
        logger.exception('Invalid JSON in sync request')

        error = JsonResponse({'ok': False, 'error': 'Invalid JSON in request body'}, status=400)
    ...
    try:
        ...
        error = JsonResponse({'ok': False, 'error': 'Malformed sync manifest'}, status=400)
...
def process_sync_request(
    ...
    if request_bytes_max < 1:
        message = f'request_bytes_max must be >= 1, got {request_bytes_max}'
    ...
    if content_type != 'application/json':
        return JsonResponse(
            {'ok': False, 'error': 'Content-Type must be application/json'}, status=415
        )
    ...
    if validate_node_id is not None:
        if not validate_node_id(request, incoming.node_id):
            return JsonResponse(
                {'ok': False, 'error': 'Node ID not authorized for this user'}, status=403
            )

    logger.info(
        'Received sync from node %s with %d payloads', incoming.node_id, len(incoming.payloads)
    )
    ...
    try:
        ...
        return JsonResponse({'ok': False, 'error': 'Sync aborted'}, status=409)

    skipped_count = sum(len(keys) for keys in result.skipped.values())

    return JsonResponse(
        {
            **response.to_dict(),
            'ok': result.ok,
            'errors': [{'key': error.key, 'message': error.message} for error in result.errors],
