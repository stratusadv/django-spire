...
class DjangoSyncStorage:
    def __init__(
        ...
        self._record_reader = record_reader or DjangoRecordReader(
            models=models, identity_field=identity_field
        )
        ...
        self._sequence_allocator = sequence_allocator or SyncSequenceAllocator()
    ...
    def delete_many(self, model_label: str, deletes: dict[str, int], origin_node: str) -> None:
        self._record_writer.delete_many(model_label, deletes, origin_node)
    ...
    def get_deletes_since(
        ...
        return self._record_reader.get_deletes_since(
            model_label, sequence, peer_node_id, sequence_max=sequence_max
        )
    ...
    def save_checkpoint(
        ...
        self._checkpoint_store.save_checkpoint(
            peer_node_id, peer_sequence, local_sequence_pushed, after_keys=after_keys
        )

    def stamp_unstamped_records(
        self, clock: HybridLogicalClock, model_order: list[str] | None = None
    ) -> None:
        count = self._record_writer.stamp_unstamped_records(clock=clock, model_order=model_order)

        if count:
            logger.info('Pre-sync stamping complete: %d records', count)

    def upsert_many(
        self, model_label: str, records: dict[str, SyncRecord], origin_node: str
    ) -> UpsertResult:
        return self._record_writer.upsert_many(model_label, records, origin_node)
