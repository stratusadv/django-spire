...
class ApiAccessModelTestCase(TestCase):
    ...
    def test_set_key_and_save(self) -> None:
        assert self.api_access.hashed_key == hash_string(self.raw_key)

        expected_hint = 'test ... 6789'
        assert self.api_access.key_hint == expected_hint

    def test_str_method(self) -> None:
        expected_str = 'Test API - test ... 6789'
        assert str(self.api_access) == expected_str

    def test_default_level(self) -> None:
        assert self.api_access.permission == ApiPermissionChoices.VIEW
