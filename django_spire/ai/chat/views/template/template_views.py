...
def dialog_widget_view(request) -> TemplateResponse:
    ...
    return TemplateResponse(
        request, context={'chat': chat}, template='django_spire/ai/chat/widget/dialog_widget.html'
    )
...
def recent_chats_widget_view(request) -> TemplateResponse:
    return TemplateResponse(
        ...
        context={
            'recent_chats': (
                Chat.objects.by_user(request.user).order_by('-last_message_datetime').active()
            )[:20]
        },
...
def search_chats_results_widget_view(request) -> TemplateResponse:
    ...
    return TemplateResponse(
        request,
        context={'search_results': [{'name': chat.name, 'id': chat.id} for chat in chats]},
