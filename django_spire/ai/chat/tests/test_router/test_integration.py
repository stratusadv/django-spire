...
class TestRouterIntegration(BaseTestCase):
    ...
    def test_direct_router_selection(self) -> None:
        result = chat_workflow(request=self.request, user_input='I need help', message_history=None)
    ...
    def test_end_to_end_workflow(self) -> None:
        ...
        result = chat_workflow(
            request=self.request, user_input='Current message', message_history=message_history
        )
    ...
    def test_workflow_handles_empty_routers(self) -> None:
        with patch(
            ...
            result = chat_workflow(request=self.request, user_input='Test', message_history=None)
    ...
    def test_workflow_with_none_message_history(self) -> None:
        result = chat_workflow(request=self.request, user_input='Test input', message_history=None)
    ...
    def test_workflow_preserves_user_input(self) -> None:
        ...
        with patch.object(
            SupportRouter, 'workflow', wraps=SupportRouter().workflow
        ) as mock_workflow:
            mock_workflow.return_value = DefaultMessageIntel(text='Response')

            chat_workflow(request=self.request, user_input=test_input, message_history=None)
