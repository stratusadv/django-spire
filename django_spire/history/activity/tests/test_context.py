...
class TestActivityUserContextManager:
    def test_sets_user_inside_block(self, user: User) -> None:
        with activity_user(user):
            assert get_current_user() == user

    def test_resets_after_block(self, user: User) -> None:
        with activity_user(user):
            pass

        assert get_current_user() is None

    def test_resets_after_exception(self, user: User) -> None:
        message = 'activity user failure'

        with pytest.raises(RuntimeError, match=message), activity_user(user):
            raise RuntimeError(message)

        assert get_current_user() is None

    def test_restores_previous_user(self, user: User) -> None:
        other_user = User(pk=2, first_name='Other', last_name='User')
        set_current_user(user)

        with activity_user(other_user):
            assert get_current_user() == other_user

        assert get_current_user() == user

    def test_none_user_hides_outer_user(self, user: User) -> None:
        set_current_user(user)
...
class TestContext:
    ...
    def test_anonymous_user_resolves_to_none(self) -> None:
...
class TestMiddleware:
    ...
    def test_user_cleared_after_exception(self, factory: RequestFactory, user: User) -> None:
        ...
        with pytest.raises(ValueError, match='response failed'):
            middleware(request)
