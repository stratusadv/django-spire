...
class HelpDeskTicketNotifications:
    ...
    def _get_helpdesk_notification_management_users(self) -> QuerySet[User]:
        return User.objects.filter(Q(groups__permissions__codename='delete_helpdeskticket') | Q(is_superuser=True)).all()
    ...
    def _create_helpdesk_notification_for_user(
            ...
            body: str,
            url: str):
        # Handle the case where the user was not passed as a User object (this means they aren't a user in the database)
        if not isinstance(user, User):
            if 'email' in user:
                user_email = user['email']
                user = None
            else:
                raise ValueError('No valid user email address found for email notification recipient. Ensure all the configured help desk email notification recipients have an email address.')
        else:
            user_email = user.email
    ...
    def send_ticket_created_notifications(self, ticket_detail_absolute_url: str):
        title = f'New Help Desk Ticket #{self.ticket.pk} has been created.'

        # Send email notifications to developers
        email_body = render_to_string(
            template_name='django_spire/help_desk/notification/email/ticket_created_email.html',
            context={
                'user_display_name': self.ticket.created_by.first_name,
                'ticket': self.ticket,
                'ticket_url': ticket_detail_absolute_url
            }
        )
        self._create_helpdesk_notification_for_users(
            ...
            title=title,
            body=email_body,
            url=ticket_detail_absolute_url
        )
