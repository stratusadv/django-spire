...
def _include_namespace_urls_with_permissions(
    ...
    permission_handler: type[ViewPermissionHandler],
):
    return [
        path(
            route=route,
            view=_view_with_permissions(
                permissions=permission_handler.url_permissions_map.get(name, None),
                view_callable=view,
                permissions_decorator=permission_handler.permissions_required_decorator
            ),
            name=name
        )
...
def include_app_urls_with_permissions(
    ...
    return [
        path(
            f'{namespace}/',
            include(
                (_include_namespace_urls_with_permissions(url_patterns, permission_handler), app_name),
                namespace=namespace
            ),
            kwargs={'permission_handler': permission_handler}
        )
