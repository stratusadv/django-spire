...
def generic_relationship_selection_prompt(
    model_class: type[Model], related_model_classes: list[type[Model]]
) -> Prompt:
    ...
    for related_model_class_map in related_model_classes_map.values():
        for related_model_object in related_model_class_map['queryset']:
            ...
            related_model_object_id_to_content_type_id_map[str(related_model_object.id)] = (
                content_type_id
            )
    ...
    return (
        ...
        .line_break()
        .text(
            'Use the following mapping to correctly set `content_type_id` based on the selected entity ID:'
        )
