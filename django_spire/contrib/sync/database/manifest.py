...
class ModelPayload:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ModelPayload:
        ...
        if not model_label:
            message = "ModelPayload requires a non-empty 'model_label'"
            raise ManifestFieldError(message)
        ...
        if not isinstance(raw_deletes, dict):
            message = "'deletes' must be a dict of {key: tombstone_ts}"
            raise ManifestFieldError(message)
...
class SyncManifest:
    ...
    def from_dict(cls, data: dict[str, Any]) -> SyncManifest:
        ...
        if checkpoint < 0:
            message = f"'checkpoint' must be non-negative, got {checkpoint}"
            raise ManifestFieldError(message)
        ...
        payloads = [ModelPayload.from_dict(payload) for payload in raw_payloads]
