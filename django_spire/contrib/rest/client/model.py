...
from django_spire.contrib.rest.client.http import RestHttpClient
...
class RestClientObjectsDescriptor(Generic[TSchema]):
    def __get__(self, obj, objtype=None) -> RestQuerySet[TSchema]:
        if obj is None:
            ...
            obj = objtype()
        return RestQuerySet[TSchema](client=obj, rest_model_class=obj.rest_model_class)


class RestModelClient(Generic[TSchema], RestHttpClient, ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not hasattr(cls, 'rest_model_class') or cls.rest_model_class is None:
            for base in cls.__orig_bases__:
                origin = get_origin(base)
                if origin is not None and issubclass(origin, RestModelClient):
                    args = get_args(base)
                    if args and isinstance(args[0], type) and issubclass(args[0], BaseModel):
                        cls.rest_model_class = args[0]
                        break
    ...
    @abstractmethod
    def fetch_one(self, **params) -> TSchema:
        """
    ...
    @abstractmethod
    def fetch_many(self, **params) -> list[TSchema]:
        """
