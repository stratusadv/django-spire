...
from django_spire.contrib.changelog.exceptions import ChangeLogModuleNotFoundError, \
...
def get_validated_changelog():
    ...
    if not isinstance(changelog, Sequence):
        message = f'ChangeLog object at {settings.DJANGO_SPIRE_CHANGELOG_MODULE} must be a valid Sequence object'
        raise InvalidChangeLogInstanceError(message)

    for entry in changelog:
        if not isinstance(entry, ChangelogEntry):
            message = f'ChangeLog object at {settings.DJANGO_SPIRE_CHANGELOG_MODULE} must only contain ChangeLogEntry objects.'
            raise InvalidChangeLogInstanceError(message)
