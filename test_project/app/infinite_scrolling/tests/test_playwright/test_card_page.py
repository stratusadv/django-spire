...
if TYPE_CHECKING:
    ...
    from test_project.app.infinite_scrolling.tests.test_playwright.pages.card_page import (
...
class TestCardPage:
    ...
    def test_multiple_scroll_containers(
        self, card_page: InfiniteScrollingCardPage, infinite_scrolling_data: list[InfiniteScrolling]
    ) -> None:
        card_page.goto_page()

        assert card_page.get_container_count() >= 2

    def test_card_scroll_independence(
        self, card_page: InfiniteScrollingCardPage, infinite_scrolling_data: list[InfiniteScrolling]
    ) -> None:
        card_page.goto_page()
