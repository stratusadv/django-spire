...
def migration_rows_view(request: WSGIRequest) -> TemplateResponse:
    ...
    sort_mapping = {'app': 'app', 'name': 'name', 'applied': 'applied'}
...
def detail_rows_view(request: WSGIRequest, task_id: int) -> TemplateResponse:
    ...
    return TemplateResponse(
        request, context=context_data, template='tabular/table/detail_rows.html'
    )
...
def user_detail_rows_view(request: WSGIRequest, user_id: int) -> TemplateResponse:
    ...
    task_users = TaskUser.objects.filter(user=user).select_related('task').annotate_user_cost()

    context_data = {'user': user, 'task_users': task_users}

    return TemplateResponse(
        request, context=context_data, template='tabular/table/user_detail_rows.html'
    )


def user_rows_view(request: WSGIRequest, task_id: int) -> TemplateResponse:
    task = get_object_or_404(Task.objects.prefetch_related('users', 'users__user'), id=task_id)

    task_users = (
        task.users.select_related('user').annotate_calculated_cost().annotate_calculated_price()
    )

    context_data = {'task': task, 'task_users': task_users}

    return TemplateResponse(request, context=context_data, template='tabular/table/user_rows.html')
